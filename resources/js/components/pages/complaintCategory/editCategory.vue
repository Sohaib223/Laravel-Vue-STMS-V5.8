...
export default {
  ...
  methods: {
    fetchData() {
      ...
      axios
        .get(_this.baseurl + "/api/complaintCategory/" + id + "/edit")
        .then(response => {
          _this.myForm.category_name = response.data.category_name;
        })
        .catch(error => {
          if (_this.rptErr < 8) {
            _this.fetchData();
    ...
    submitForm(e) {
      ...
      axios
        .patch(_this.baseurl + "/api/complaintCategory/" + id, _this.myForm)
        .then(response => {
