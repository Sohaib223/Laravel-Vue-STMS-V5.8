        ...
        <div class="col-md-12">
          <div class="block">
            <div class="block-header block-header-default">
              <h3 class="block-title">Edit notification</h3>
              <div class="block-options">
                <router-link to="/notification" class="btn btn-info">Manage Categories</router-link>
                <router-link to="/notification/create" class="btn btn-info">Add Categories</router-link>
              </div>
            </div>
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      axios
        .get(_this.baseurl + "/api/notification/" + id + "/edit")
        .then(response => {
          $(document).ready(function() {
            $(".location-selector").select2("destroy");
            $(".location-selector").val(response.data.notification.location_id);
          });

          _this.myForm.title = response.data.notification.title;
          _this.myForm.description = response.data.notification.description;
          _this.myForm.address = response.data.notification.address;

          _this.myForm.location_id = response.data.notification.location_id;
    ...
    submitForm(e) {
      ...
      axios
        .patch(_this.baseurl + "/api/notification/" + id, _this.myForm)
        .then(response => {
          if (response.data * 1 === 1) {
            ...
            //_this.$router.push({path : '/notification'});
